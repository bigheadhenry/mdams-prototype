...
截至 **2026-05-17**，MDAMS 与 OAIS 的关系最准确的描述仍是：
...
- BagIt SIP-like 打包层；
- 原件、访问表示、导出表示之间的分层；
- 三维对象 / 版本 / 文件包和视频来源接入带来的多模态资源分层；
- AI 与人脸识别等辅助能力带来的元数据增强和交互增强线索；
- 研究线中对保存意识和边界的持续说明。
...
flowchart LR
    ...
    Storage["OAIS: Archival Storage"] --> MDAMSStorage["MDAMS: 文件存储 + access 副本 + BagIt ZIP + 三维/视频文件\n但非完整归档存储体系"]
    DataMgmt["OAIS: Data Management"] --> MDAMSData["MDAMS: Asset / ImageRecord / ThreeD / Video / Platform metadata\n但非完整 OAIS 信息模型"]
    Access["OAIS: Access"] --> MDAMSAccess["MDAMS: IIIF Manifest + 后端图像服务代理 + Mirador + 三维 viewer + 视频 stream + 下载"]
...
| Archival Storage | 文件存储、访问副本、BagIt ZIP、三维/视频文件 | 部分 | 有保存导向痕迹，但不是完整归档存储体系 |
| Data Management | `Asset`、`ImageRecord`、三维 metadata、视频 metadata、平台视图 | 部分 | 有对象和元数据管理，但未 formalize 为 OAIS 信息模型 |
| Access | IIIF Manifest、后端 IIIF service 代理、Mirador、普通下载、三维 viewer、视频 stream | 明显概念对齐 | 访问层是当前最成熟的对外表现层 |
...
| AIP | 无正式一等实体 | 当前不在范围内 |
| DIP | IIIF Manifest、普通下载、申请交付包、三维 viewer 表现、视频 stream 等输出对象 | 有输出层现实，但未以 OAIS DIP 术语 formalize |
...
### 3. 正式 OAIS 信息模型映射
当前仍没有把 `Asset`、`ImageRecord`、三维对象、视频资源、导出包一一映射成 OAIS 信息对象体系。
...
### 稍长版
MDAMS Prototype 与 OAIS 的关系主要体现在概念层和局部实现层：系统采用了 SIP-like 的 ingest 语言、区分原始对象与访问表示、提供 preservation-aware 的 BagIt 打包输出，并围绕 fixity、元数据提取、转换、访问、多模态来源聚合和交付建立了生命周期导向的工作流。然而，系统尚未把 SIP/AIP/DIP 建模为完整实体，也未覆盖 OAIS 的全部职能域，因此更适合被描述为一个受 OAIS 思维启发的研究型原型，而不是完整 OAIS 仓储。

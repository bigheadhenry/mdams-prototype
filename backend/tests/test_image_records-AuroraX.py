...
def test_business_activity_confirm_bind_enqueues_face_recognition(db_session, test_upload_dir, monkeypatch):
    monkeypatch.setattr(app_config, "UPLOAD_DIR", str(test_upload_dir))
    monkeypatch.setattr(app_config, "FACE_RECOGNITION_ENABLED", True)
    monkeypatch.setattr(app_config, "FACE_RECOGNITION_THRESHOLD", 0.66)
    seed_auth_data(db_session)
    metadata_user = _metadata_entry_user()
    photographer_user = _photographer_user()
    photographer_user_id = _photographer_user_id(db_session)
    ...
    created = image_records_router.create_image_record(
        payload=ImageRecordSaveRequest(
            title="Business Activity Photo",
            profile_key="business_activity",
            management={"project_name": "Activity Plan", "image_category": "business_activity"},
            profile_fields={"main_location": "东华门"},
            assigned_photographer_user_id=photographer_user_id,
        ),
        db=db_session,
        user=metadata_user,
    ...
    derivative_enqueued: list[tuple[int, str]] = []
    monkeypatch.setattr(
        image_records_router.generate_iiif_access_derivative,
        "delay",
        lambda asset_id, source_path: derivative_enqueued.append((asset_id, source_path)),
    )
    face_enqueued: list[tuple[int, int]] = []
    monkeypatch.setattr(
        image_records_router.recognize_business_activity_faces,
        "delay",
        lambda record_id, asset_id: face_enqueued.append((record_id, asset_id)),
    )

    detailed = asyncio.run(
        image_records_router.upload_temp_image_for_record(
            record_id=created.id,
            file=_make_png_upload("activity-photo.png"),
            db=db_session,
            user=photographer_user,
        )
    )
    ...
    face_recognition = confirmed.metadata_info["raw_metadata"]["face_recognition"]
    assert confirmed.asset is not None
    assert face_enqueued == [(created.id, confirmed.asset.asset_id)]
    assert face_recognition["status"] == "pending"
    assert face_recognition["threshold"] == 0.66
    assert face_recognition["asset_id"] == confirmed.asset.asset_id


def test_face_recognition_task_updates_business_activity_main_person(db_session, test_upload_dir, monkeypatch):
    monkeypatch.setattr(app_config, "UPLOAD_DIR", str(test_upload_dir))
    monkeypatch.setattr(app_config, "FACE_RECOGNITION_ENABLED", True)
    monkeypatch.setattr(app_config, "FACE_RECOGNITION_THRESHOLD", 0.55)
    seed_auth_data(db_session)
    metadata_user = _metadata_entry_user()
    photographer_user = _photographer_user()
    photographer_user_id = _photographer_user_id(db_session)
    ...
    created = image_records_router.create_image_record(
        payload=ImageRecordSaveRequest(
            title="Activity Group Photo",
            profile_key="business_activity",
            management={"project_name": "Activity Plan", "image_category": "business_activity"},
            profile_fields={"main_location": "箭亭"},
            assigned_photographer_user_id=photographer_user_id,
        ),
        db=db_session,
        user=metadata_user,
    ...
    monkeypatch.setattr(image_records_router.generate_iiif_access_derivative, "delay", lambda *_args: None)
    monkeypatch.setattr(image_records_router.recognize_business_activity_faces, "delay", lambda *_args: None)

    detailed = asyncio.run(
        image_records_router.upload_temp_image_for_record(
            record_id=created.id,
            file=_make_png_upload("activity-group.png"),
            db=db_session,
            user=photographer_user,
        )
    )
    ...
    monkeypatch.setattr(
        ...
        lambda _file_path, threshold=None, request_id=None: {
            "status": "success",
            "count": 3,
            "results": [
                {
                    ...
                    "bbox": [10, 20, 60, 90],
                    "recognized": True,
                    "person_info": {"id": "cluster-zhang", "name": "张三"},
                    "confidence": 0.91,
                    "score": 0.88,
                },
                {
                    ...
                    "bbox": [80, 25, 140, 105],
                    "recognized": True,
                    "person_info": {"id": "cluster-li", "name": "李四"},
                    "confidence": 0.89,
                    "score": 0.84,
                },
                {
    ...
    app_tasks.recognize_business_activity_faces.run(created.id, confirmed.asset.asset_id)

    refreshed_record = db_session.query(image_records_router.ImageRecord).filter(image_records_router.ImageRecord.id == created.id).one()
    refreshed_asset = db_session.query(Asset).filter(Asset.id == confirmed.asset.asset_id).one()
    record_face_recognition = refreshed_record.metadata_info["raw_metadata"]["face_recognition"]
    asset_face_recognition = refreshed_asset.metadata_info["raw_metadata"]["face_recognition"]

    assert refreshed_record.metadata_info["profile"]["fields"]["main_person"] == "张三、李四"
    assert record_face_recognition["status"] == "success"
    assert record_face_recognition["recognized_names"] == ["张三", "李四"]
    assert record_face_recognition["face_count"] == 3
    assert len(record_face_recognition["faces"]) == 3
    assert asset_face_recognition["recognized_names"] == ["张三", "李四"]
